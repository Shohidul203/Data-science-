...
ggplot(mydata, aes(x = ph, fill = Potability)) +
  geom_density(alpha = 0.5) +
  labs(title = "Density Plot of pH by Potability", x = "pH", y = "Density", fill = "Potability") +
  facet_wrap(~ Potability)

ggplot(mydata, aes(x = ph, fill = Potability)) +
  geom_histogram(alpha = 0.5, position = "identity", bins = 30) +
  labs(title = "HISTOGRAM OF PH BY POTABILITY", x = "PH", y = "FREQUENCY", fill = "POTABILITY") +
  facet_wrap(~ Potability)


# Create the violin plot
ggplot(mydata, aes(x = Potability, y = ph, fill = Potability)) +
  geom_violin() +
  labs(x = "Potability", y = "pH", title = "Violin Plot of pH by Potability") +
...
pairs(numeric_data)
#ECDF
ggplot(mydata, aes(x = Sulfate)) +
  stat_ecdf() +
  labs(title = "ECDF Plot of pH")
#violin plot
